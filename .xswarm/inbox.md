# Inbox

_Requests to this project's agents. One per `## ` heading._

## Re: DRBI web page for Sunday Concert Series
- **Re: DRBI web page for Sunday Concert Series** — Oraea sent a mockup for the First Sunday Concert Series page and is waiting to supply the header image and rose music logo so the page can be built on the DRBI site.  
  _oraeav@gmail.com · this week_

## Re: Document shared with you: "DRBI-Letter-of-Intent.md"
- **Re: Document shared with you: "DRBI-Letter-of-Intent.md"** — Taj approves the MOU but wants every board member to read and mark up the Letter of Intent before a final draft goes out, rather than Chad sending it to Danny and Flora today as planned.  
  _tajsabet@gmail.com · today_

## Re: DRBI web page for Sunday Concert Series
- **Re: DRBI web page for Sunday Concert Series** — Oraea sent a mockup for the First Sunday Concert Series page and is waiting to send the header image and rose music logo, plus the full-size PDF Chad requested.  
  _oraeav@gmail.com · this week_

## New Form Submission from your Website
- **New Form Submission from your Website** — A contact-form submission from the DRBI site asks whether the family must use a particular funeral home for a Desert Rose Memorial Cemetery burial or may choose any in the Scottsdale/Phoenix area.  
  _notify+l0e9xu@web3forms.com (Taraz Samandari) · today_

## Re: Document shared with you: "DRBI-Letter-of-Intent.md"
- **Re: Document shared with you: "DRBI-Letter-of-Intent.md"** — Pete cannot open the shared DRBI-Letter-of-Intent.md and needs it resent in a format he can read so he can approve it.  
  _pete@columbiafutures.com · today_

## Disregard previous version DRBI web page for Sunday Concert Series
- **Disregard previous version DRBI web page for Sunday Concert Series** — Oraea sent the corrected full-size flyer renders and PNG assets to be built into a First Sunday Concert Series page on the DRBI site.  
  _oraeav@gmail.com · this week_

## New Form Submission from your Website
- **New Form Submission from your Website** — A family with a plot at Desert Rose Memorial Cemetery asks whether they must use a particular funeral home or may choose any in the Scottsdale/Phoenix area, and needs a reply.  
  _notify+l0e9xu@web3forms.com (Taraz Samandari via drbi.org contact form) · today_

## Re: Document shared with you: "DRBI-Letter-of-Intent.md"
- **Re: Document shared with you: "DRBI-Letter-of-Intent.md"** — Pete cannot open the shared DRBI Letter of Intent and needs it resent in a format he can read.  
  _pete@columbiafutures.com · today_

## Re: Desert Rose — moving forward with your proposal
- **Re: Desert Rose — moving forward with your proposal** — Flanny Rentals sent a recap of what they need to start: Evolve login, Airbnb login, and the calendar of all existing bookings.  
  _teamflannyrentals@gmail.com · this week_

## Disregard previous version DRBI web page for Sunday Concert Series
- **Disregard previous version DRBI web page for Sunday Concert Series** — Oraea sent the corrected full-size flyer renders and PNG assets for the DRBI First Sunday Concert Series page so the page can be built.  
  _oraeav@gmail.com · this week_

## Sunday Series-DRBI.org
- **Sunday Series-DRBI.org** — Oraea Varis sent the confirmed First Sunday Concert Series schedule (Oct 2026–Mar 2027, plus TBD April/May) with bios and photos where available, asking Chad to build the series landing page and individual concert posts on drbi.org now and add the rest as they firm up.  
  _oraeav@gmail.com · this week_
