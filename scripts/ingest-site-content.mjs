...
// Flatten one level; nested objects/arrays are stored as JSON strings.
function collectSettings() {
  const json = JSON.parse(readFileSync(join(ROOT, 'src/data/site.json'), 'utf8'));
  return Object.entries(json).map(([k, v]) => ({
    name: qs(`site.${k}`),
    value: qs(typeof v === 'string' ? v : JSON.stringify(v)),
    _key: k,
  }));
}
...
const sql = [
  ...
  `-- Generated ${new Date().toISOString().slice(0, 10)} from ${pages.length} pages, ` +
    `${settings.length} settings, ${links.length} links, ${board.length} people.`,
  ...
  '',
  rowsToInsert('options', ['name', 'value'], settings),
  '',
...
console.log(`settings       ${settings.length}  from site.json -> options as site.*`);
